Feature: REST API - Cards
  ...
  Scenario: PUT /boards/{boardId}/stacks/{stackId}/cards/{cardId}/reorder - Move a card to another stack
    Given sending "POST" to the API endpoint "/boards/<boardId>/stacks" with body:
      | title | Doing |
      | order | 2     |
    And the response value "id" is stored as "targetStackId"
    And sending "POST" to the API endpoint "/boards/<boardId>/stacks/<stackId>/cards" with body:
      | title | Card to move |
      | type  | plain        |
      | order | 999          |
    And the response value "id" is stored as "cardId"
    # The stack in the path is the one the card sits in, the body holds the target
    When sending "PUT" to the API endpoint "/boards/<boardId>/stacks/<stackId>/cards/<cardId>/reorder" with body:
    ...
    When sending "GET" to the API endpoint "/boards/<boardId>/stacks/<stackId>/cards/<cardId>"
    Then the response should have a status code "200"
    And the response value "stackId" should be "<targetStackId>"
    When sending "GET" to the API endpoint "/boards/<boardId>/stacks/<targetStackId>"
    Then the response value "cards" should have 1 entry
    And the response value "cards.0.title" should be "Card to move"
