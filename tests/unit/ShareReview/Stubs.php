...
namespace OCP\Share\ShareReview {
	...
	if (!class_exists(ShareReviewPermission::class)) {
	...
	if (!class_exists(ShareReviewEntry::class)) {
	...
	if (!interface_exists(IShareReviewSourceRemediation::class)) {
	...
	if (!interface_exists(IShareReviewSourceSnapshot::class)) {
	...
	if (!class_exists(ShareReviewQuery::class)) {
		final class ShareReviewQuery {
			...
			public function __construct(
				...
				public readonly ?int $modifiedBeforeTimestamp = null,
			...
			public function isFiltered(): bool {
				return $this->search !== null
					...
					|| $this->modifiedBeforeTimestamp !== null
	...
	if (!class_exists(ShareReviewActionContext::class)) {
		final readonly class ShareReviewActionContext {
			public function __construct(
				public ?string $actingUserId = null,
				public string $scope = Events\ShareReviewAccessCheckEvent::SCOPE_OPERATOR,
			) {
			}
		}
	}

	if (!class_exists(ShareReviewCounts::class)) {
	...
	if (!class_exists(ShareReviewPage::class)) {
...
namespace OCP\Share\ShareReview\Events {
	...
	if (!class_exists(ShareReviewAccessCheckEvent::class)) {
